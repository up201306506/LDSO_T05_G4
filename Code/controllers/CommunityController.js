...
var insertCommunity = function (db, name, headOffice, category, founder, description, privacy, coin, creationDate,members,callback){
    ...
    community.find({name:name, date: creationDate }).toArray(function (err,docs) {
        ...
        if(docs.length>=1){
            ...
            community.insertOne({name: name, office: headOffice, category: category, description: description, date: creationDate, founder: founder, privacy: privacy, has_coin: coin, members: members},
                function (err,result) {
                    assert.equal(err,null);
