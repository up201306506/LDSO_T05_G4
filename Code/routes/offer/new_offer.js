...
router.post('/', upload.single("offer_image"), function(req, res, next) {
    ...
    mongo.connect(configDB.url, function (err, db, next) {
        offerController.insertOffer(db, req.body.optradio, req.user, req.body.offer_description, false, Date.now(), req.body.offer_price, req.body.offer_name, req.file.filename,  function (result, id) {
            db.close();
